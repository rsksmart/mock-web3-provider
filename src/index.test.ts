...
describe('default provider', function (this: {
  ...
  describe('eth_accounts & eth_requestAccounts', () => {
    ...
    it('is correct with request', async () => {
      const response = await this.provider.request({ method: 'eth_accounts', params: [] })
      expect(response).toEqual([address])

      const response2 = await this.provider.request({ method: 'eth_requestAccounts', params: [] })
      expect(response2).toEqual([address])
    })

    it('is correct with sendAsync', () => {
      this.provider.sendAsync({ method: 'eth_accounts' }, (_err: Error, success: string[]) => expect(success).toEqual({ result: [address] }))
    })
  })
  ...
  describe('eth_chainId & net_version', () => {
    ...
    it('is correct with response', async () => {
      const response = await this.provider.request({ method: 'net_version', params: [] })
      expect(response).toEqual(31)

      const response2 = await this.provider.request({ method: 'eth_chainId', params: [] })
      expect(response2).toEqual('0x1f')
    })

    it('is correct with sendAsync', () => {
      this.provider.sendAsync({ method: 'net_version' }, (_err: Error, success: number) => expect(success).toEqual({ result: 31 }))
    })
  })

  describe('personal_sign', () => {
    it('returns the correct signature for the message', async () => {
      const signed = await this.provider.request({ method: 'personal_sign', params: ['hello world!'] })
  ...
  describe('eth_decrypt', () => {
    it('decrypts content', async () => {
      ...
      const result = await this.provider.request({ method: 'eth_decrypt', params: [encrypted, address] })
